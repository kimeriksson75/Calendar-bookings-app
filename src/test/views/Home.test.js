...
describe('Home with no upcoming user bookings', () => {
	beforeEach(() => {
		const mockStore = configureStore([]);
		const store = mockStore({
			auth: {
			...
			bookingData: {
				userBookings: [{
					_id: '6580a01ffaa7d604c93901ce',
					service: '656b75c56c0a78cef01c9cf0',
					date: '2023-11-31T19:40:13.904Z',
					alternateTimeslots: [
						{
							_id: '5f9a7b8f3f5a6a1a0c5b6f2f',
							userid: '658055edfaa7d604c938ec48',
							username: 'johndoe',
							start: '2023-11-02T07:00:00.000Z',
					...
					timeslots: [
						...
						},
						{
							...
							userid: null,
							username: '',
							start: '2023-11-02T19:30:00.000Z',
							end: '2023-11-02T22:00:00.000Z',
							__v: 0,
						},
					],
	...
	it('should not render upcoming user bookings widget', async () => {
		const upcomingUserBookings = screen.queryByTestId('upcoming-user-bookings');
		expect(upcomingUserBookings).not.toBeInTheDocument();
	});
	
	it('should not render next user booking widget', async () => {
		const nextUserBooking = screen.queryByTestId('next-user-booking');
		expect(nextUserBooking).not.toBeInTheDocument();
		expect(screen.getByText(/Du har inga kommande bokningar/)).toBeInTheDocument();
	});
